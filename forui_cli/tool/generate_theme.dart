...
final _theme = p.join(library, 'src', 'theme', 'theme.dart');
...
String generateThemes(ThemeFragment fragment) {
  String literal(String source) => literalString(source.replaceAll(r'$', r'\$')).accept(emitter).toString();

  final library = LibraryBuilder()
    ..comments.addAll(header)
    ..body.add(
      (FieldBuilder()
            ..docs.addAll(['/// The neutral theme. Generated by tool/main.dart.'])
            ..name = 'neutral'
            ..modifier = .constant
            ..assignment = Code(
              '(\n'
              '  source: ${literal(fragment.theme)},\n'
              '  colors: ${literal(fragment.colors)},\n'
              '  typography: ${literal(fragment.typography)},\n'
              '  style: ${literal(fragment.style)},\n'
              '  icons: ${literal(fragment.icons)},\n'
              ')',
            ))
          .build(),
    );
...
ThemeFragment mapThemes(ThemesConstructors themes) {
  ...
  for (final constructor in themes.themes) {
...
Future<ThemesConstructors> traverseThemes(AnalysisContextCollection collection) async {
  ...
  if (await collection.contextFor(_theme).currentSession.getResolvedUnit(_theme) case final ResolvedUnitResult result) {
  ...
  throw Exception('Failed to parse $_colors & $_theme');
}
          ...
          if (expression case final InstanceCreationExpression platformTheme) {
            ...
            for (final NamedExpression(:name, :expression)
              ...
              if (themeDataExpr case final InstanceCreationExpression themeData) {
                ...
                themes.add((theme: theme, variant: variant, colors: colors));
              }
            }
          }
        }
