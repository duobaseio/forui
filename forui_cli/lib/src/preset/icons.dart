...
String generateIcons(Preset preset) {
  ...
  for (final MapEntry(key: slot, value: icon) in preset.icon._mappings.entries) {
    icons = icons.replaceAll('FIcons.iconData(FLucideIcons.$slot)', icon);
  }
...
Future<void> installIcons(Preset preset, Directory root) async {
  ...
  final progress = spinner()..start('Adding ${preset.icon.package}...');
  final result = await Process.run('flutter', ['pub', 'add', preset.icon.package], workingDirectory: root.path);

  if (result.exitCode != 0) {
    progress.error('Failed to add ${preset.icon.package}.\n${result.stderr}');
    exit(1);
  }

  progress.stop('Added ${preset.icon.package}');
}
...
extension Icons on IconLibrary {
  ...
  /// Maps each Forui icon slot to the equivalent icon in this library.
  Map<String, String> get _mappings => switch (this) {
    .lucide => const {},
    .tabler => {
