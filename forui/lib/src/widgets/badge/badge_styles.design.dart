...
/// A delta that applies modifications to a [FBadgeStyles].
sealed class FBadgeStylesDelta with Delta<FBadgeStyles> {
  /// Creates a complete replacement for a [FBadgeStyles].
  const factory FBadgeStylesDelta.replace(FBadgeStyles replacement) = _FBadgeStylesReplace;

  /// Creates a partial modification of a [FBadgeStyles].
  const factory FBadgeStylesDelta.merge({
    FBadgeStyleDelta? primary,
    FBadgeStyleDelta? secondary,
    FBadgeStyleDelta? outline,
    FBadgeStyleDelta? destructive,
  }) = _FBadgeStylesMerge;
}

class _FBadgeStylesReplace implements FBadgeStylesDelta {
  const _FBadgeStylesReplace(this._replacement);

  final FBadgeStyles _replacement;

  @override
  FBadgeStyles call(FBadgeStyles _) => _replacement;
}

class _FBadgeStylesMerge implements FBadgeStylesDelta {
  const _FBadgeStylesMerge({this.primary, this.secondary, this.outline, this.destructive});

  final FBadgeStyleDelta? primary;

  final FBadgeStyleDelta? secondary;

  final FBadgeStyleDelta? outline;

  final FBadgeStyleDelta? destructive;

  @override
  FBadgeStyles call(FBadgeStyles original) => FBadgeStyles(
