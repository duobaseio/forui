...
sealed class FPickerStyleDelta with Delta<FPickerStyle> {
  /// Creates a complete replacement for a [FPickerStyle].
  const factory FPickerStyleDelta.replace(FPickerStyle replacement) = _FPickerStyleReplace;

  /// Creates a partial modification of a [FPickerStyle].
  const factory FPickerStyleDelta.merge({
    double? diameterRatio,
    double? squeeze,
    double? magnification,
    double? overAndUnderCenterOpacity,
...
class _FPickerStyleReplace implements FPickerStyleDelta {
  const _FPickerStyleReplace(this._replacement);

  final FPickerStyle _replacement;

  @override
  FPickerStyle call(FPickerStyle _) => _replacement;
}

class _FPickerStyleMerge implements FPickerStyleDelta {
  const _FPickerStyleMerge({
    this.diameterRatio,
    this.squeeze,
    this.magnification,
