...
sealed class FProgressStyleDelta with Delta<FProgressStyle> {
  /// Creates a complete replacement for a [FProgressStyle].
  const factory FProgressStyleDelta.replace(FProgressStyle replacement) = _FProgressStyleReplace;

  /// Creates a partial modification of a [FProgressStyle].
  const factory FProgressStyleDelta.merge({
    BoxConstraints? constraints,
    BoxDecorationDelta? trackDecoration,
    BoxDecorationDelta? fillDecoration,
    FProgressMotionDelta? motion,
  }) = _FProgressStyleMerge;
}

class _FProgressStyleReplace implements FProgressStyleDelta {
  const _FProgressStyleReplace(this._replacement);

  final FProgressStyle _replacement;

  @override
  FProgressStyle call(FProgressStyle _) => _replacement;
}

class _FProgressStyleMerge implements FProgressStyleDelta {
  const _FProgressStyleMerge({this.constraints, this.trackDecoration, this.fillDecoration, this.motion});
...
sealed class FProgressMotionDelta with Delta<FProgressMotion> {
  /// Creates a complete replacement for a [FProgressMotion].
  const factory FProgressMotionDelta.replace(FProgressMotion replacement) = _FProgressMotionReplace;

  /// Creates a partial modification of a [FProgressMotion].
  const factory FProgressMotionDelta.merge({Duration? period, Duration? interval, Curve? curve, double? value}) =
      _FProgressMotionMerge;
}

class _FProgressMotionReplace implements FProgressMotionDelta {
  const _FProgressMotionReplace(this._replacement);

  final FProgressMotion _replacement;

  @override
  FProgressMotion call(FProgressMotion _) => _replacement;
}

class _FProgressMotionMerge implements FProgressMotionDelta {
  const _FProgressMotionMerge({this.period, this.interval, this.curve, this.value});
