...
sealed class FPopoverMotionDelta with Delta<FPopoverMotion> {
  /// Creates a complete replacement for a [FPopoverMotion].
  const factory FPopoverMotionDelta.replace(FPopoverMotion replacement) = _FPopoverMotionReplace;

  /// Creates a partial modification of a [FPopoverMotion].
  const factory FPopoverMotionDelta.merge({
    Duration? entranceDuration,
    Duration? exitDuration,
    Curve? expandCurve,
    Curve? collapseCurve,
    ...
    Animatable<double>? scaleTween,
    Animatable<double>? fadeTween,
  }) = _FPopoverMotionMerge;
}

class _FPopoverMotionReplace implements FPopoverMotionDelta {
  const _FPopoverMotionReplace(this._replacement);

  final FPopoverMotion _replacement;

  @override
  FPopoverMotion call(FPopoverMotion _) => _replacement;
}

class _FPopoverMotionMerge implements FPopoverMotionDelta {
  const _FPopoverMotionMerge({
...
sealed class FPopoverStyleDelta with Delta<FPopoverStyle> {
  /// Creates a complete replacement for a [FPopoverStyle].
  const factory FPopoverStyleDelta.replace(FPopoverStyle replacement) = _FPopoverStyleReplace;

  /// Creates a partial modification of a [FPopoverStyle].
  const factory FPopoverStyleDelta.merge({
...
class _FPopoverStyleReplace implements FPopoverStyleDelta {
  const _FPopoverStyleReplace(this._replacement);

  final FPopoverStyle _replacement;

  @override
  FPopoverStyle call(FPopoverStyle _) => _replacement;
}

class _FPopoverStyleMerge implements FPopoverStyleDelta {
  const _FPopoverStyleMerge({this.decoration, this.barrierFilter, this.backgroundFilter, this.viewInsets});
