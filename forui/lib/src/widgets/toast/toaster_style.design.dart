...
sealed class FToasterStyleDelta with Delta<FToasterStyle> {
  /// Creates a complete replacement for a [FToasterStyle].
  const factory FToasterStyleDelta.replace(FToasterStyle replacement) = _FToasterStyleReplace;

  /// Creates a partial modification of a [FToasterStyle].
  const factory FToasterStyleDelta.merge({
    double? max,
    EdgeInsetsGeometry? padding,
    FToasterExpandBehavior Function()? expandBehavior,
    Duration? expandHoverEnterDuration,
    Duration? expandHoverExitDuration,
    double? expandStartSpacing,
    double? expandSpacing,
...
class _FToasterStyleReplace implements FToasterStyleDelta {
  const _FToasterStyleReplace(this._replacement);

  final FToasterStyle _replacement;

  @override
  FToasterStyle call(FToasterStyle _) => _replacement;
}

class _FToasterStyleMerge implements FToasterStyleDelta {
  const _FToasterStyleMerge({
    this.max,
    this.padding,
    this.expandBehavior,
  ...
  final FToasterExpandBehavior Function()? expandBehavior;
  ...
  FToasterStyle call(FToasterStyle original) => FToasterStyle(
    ...
    expandBehavior: expandBehavior == null ? original.expandBehavior : expandBehavior!(),
...
sealed class FToasterMotionDelta with Delta<FToasterMotion> {
  /// Creates a complete replacement for a [FToasterMotion].
  const factory FToasterMotionDelta.replace(FToasterMotion replacement) = _FToasterMotionReplace;

  /// Creates a partial modification of a [FToasterMotion].
  const factory FToasterMotionDelta.merge({
    Duration? expandDuration,
    Duration? collapseDuration,
    Curve? expandCurve,
    Curve? collapseCurve,
  }) = _FToasterMotionMerge;
}

class _FToasterMotionReplace implements FToasterMotionDelta {
  const _FToasterMotionReplace(this._replacement);

  final FToasterMotion _replacement;

  @override
  FToasterMotion call(FToasterMotion _) => _replacement;
}

class _FToasterMotionMerge implements FToasterMotionDelta {
  const _FToasterMotionMerge({this.expandDuration, this.collapseDuration, this.expandCurve, this.collapseCurve});
...
sealed class FToastStyleDelta with Delta<FToastStyle> {
  /// Creates a complete replacement for a [FToastStyle].
  const factory FToastStyleDelta.replace(FToastStyle replacement) = _FToastStyleReplace;

  /// Creates a partial modification of a [FToastStyle].
  const factory FToastStyleDelta.merge({
    BoxConstraints? constraints,
...
class _FToastStyleReplace implements FToastStyleDelta {
  const _FToastStyleReplace(this._replacement);

  final FToastStyle _replacement;

  @override
  FToastStyle call(FToastStyle _) => _replacement;
}

class _FToastStyleMerge implements FToastStyleDelta {
  const _FToastStyleMerge({
    this.constraints,
    this.decoration,
...
sealed class FToastMotionDelta with Delta<FToastMotion> {
  /// Creates a complete replacement for a [FToastMotion].
  const factory FToastMotionDelta.replace(FToastMotion replacement) = _FToastMotionReplace;

  /// Creates a partial modification of a [FToastMotion].
  const factory FToastMotionDelta.merge({
    Duration? entranceDuration,
    Duration? dismissDuration,
    Duration? transitionDuration,
    Duration? reentranceDuration,
...
class _FToastMotionReplace implements FToastMotionDelta {
  const _FToastMotionReplace(this._replacement);

  final FToastMotion _replacement;

  @override
  FToastMotion call(FToastMotion _) => _replacement;
}

class _FToastMotionMerge implements FToastMotionDelta {
  const _FToastMotionMerge({
