...
sealed class FSidebarItemStyleDelta with Delta<FSidebarItemStyle> {
  /// Creates a complete replacement for a [FSidebarItemStyle].
  const factory FSidebarItemStyleDelta.replace(FSidebarItemStyle replacement) = _FSidebarItemStyleReplace;

  /// Creates a partial modification of a [FSidebarItemStyle].
  const factory FSidebarItemStyleDelta.merge({
    FWidgetStateMap<TextStyle>? textStyle,
    double? iconSpacing,
    FWidgetStateMap<IconThemeData>? iconStyle,
    double? collapsibleIconSpacing,
  ...
  }) = _FSidebarItemStyleMerge;
}

class _FSidebarItemStyleReplace implements FSidebarItemStyleDelta {
  const _FSidebarItemStyleReplace(this._replacement);

  final FSidebarItemStyle _replacement;

  @override
  FSidebarItemStyle call(FSidebarItemStyle _) => _replacement;
}

class _FSidebarItemStyleMerge implements FSidebarItemStyleDelta {
  const _FSidebarItemStyleMerge({
    this.textStyle,
    this.iconSpacing,
    this.iconStyle,
  ...
  FSidebarItemStyle call(FSidebarItemStyle original) => FSidebarItemStyle(
    ...
    collapsibleIconStyle: collapsibleIconStyle ?? original.collapsibleIconStyle,
    childrenSpacing: childrenSpacing ?? original.childrenSpacing,
    childrenPadding: childrenPadding ?? original.childrenPadding,
...
sealed class FSidebarItemMotionDelta with Delta<FSidebarItemMotion> {
  /// Creates a complete replacement for a [FSidebarItemMotion].
  const factory FSidebarItemMotionDelta.replace(FSidebarItemMotion replacement) = _FSidebarItemMotionReplace;

  /// Creates a partial modification of a [FSidebarItemMotion].
  const factory FSidebarItemMotionDelta.merge({
    Duration? expandDuration,
    Duration? collapseDuration,
    Curve? expandCurve,
    Curve? collapseCurve,
    ...
    Animatable<double>? fadeTween,
    Animatable<double>? iconTween,
  }) = _FSidebarItemMotionMerge;
}

class _FSidebarItemMotionReplace implements FSidebarItemMotionDelta {
  const _FSidebarItemMotionReplace(this._replacement);

  final FSidebarItemMotion _replacement;

  @override
  FSidebarItemMotion call(FSidebarItemMotion _) => _replacement;
}

class _FSidebarItemMotionMerge implements FSidebarItemMotionDelta {
  const _FSidebarItemMotionMerge({
    this.expandDuration,
