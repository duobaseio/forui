...
/// A delta that applies modifications to a [FSliderStyles].
sealed class FSliderStylesDelta with Delta<FSliderStyles> {
  /// Creates a complete replacement for a [FSliderStyles].
  const factory FSliderStylesDelta.replace(FSliderStyles replacement) = _FSliderStylesReplace;

  /// Creates a partial modification of a [FSliderStyles].
  const factory FSliderStylesDelta.merge({FSliderStyleDelta? horizontalStyle, FSliderStyleDelta? verticalStyle}) =
      _FSliderStylesMerge;
}

class _FSliderStylesReplace implements FSliderStylesDelta {
  const _FSliderStylesReplace(this._replacement);

  final FSliderStyles _replacement;

  @override
  FSliderStyles call(FSliderStyles _) => _replacement;
}

class _FSliderStylesMerge implements FSliderStylesDelta {
  const _FSliderStylesMerge({this.horizontalStyle, this.verticalStyle});

  final FSliderStyleDelta? horizontalStyle;

  final FSliderStyleDelta? verticalStyle;

  @override
  FSliderStyles call(FSliderStyles original) => FSliderStyles(
...
sealed class FSliderStyleDelta with Delta<FSliderStyle> {
  /// Creates a complete replacement for a [FSliderStyle].
  const factory FSliderStyleDelta.replace(FSliderStyle replacement) = _FSliderStyleReplace;

  /// Creates a partial modification of a [FSliderStyle].
  const factory FSliderStyleDelta.merge({
    FWidgetStateMap<Color>? activeColor,
    FWidgetStateMap<Color>? inactiveColor,
    BorderRadius? borderRadius,
    double? crossAxisExtent,
...
class _FSliderStyleReplace implements FSliderStyleDelta {
  const _FSliderStyleReplace(this._replacement);

  final FSliderStyle _replacement;

  @override
  FSliderStyle call(FSliderStyle _) => _replacement;
}

class _FSliderStyleMerge implements FSliderStyleDelta {
  const _FSliderStyleMerge({
    this.activeColor,
    this.inactiveColor,
    this.borderRadius,
  ...
  FSliderStyle call(FSliderStyle original) => FSliderStyle(
    activeColor: activeColor ?? original.activeColor,
    inactiveColor: inactiveColor ?? original.inactiveColor,
