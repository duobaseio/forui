...
sealed class FSliderMarkStyleDelta with Delta<FSliderMarkStyle> {
  /// Creates a complete replacement for a [FSliderMarkStyle].
  const factory FSliderMarkStyleDelta.replace(FSliderMarkStyle replacement) = _FSliderMarkStyleReplace;

  /// Creates a partial modification of a [FSliderMarkStyle].
  const factory FSliderMarkStyleDelta.merge({
    FWidgetStateMap<Color>? tickColor,
    double? tickSize,
    FWidgetStateMap<TextStyle>? labelTextStyle,
    AlignmentGeometry? labelAnchor,
    double? labelOffset,
  }) = _FSliderMarkStyleMerge;
}

class _FSliderMarkStyleReplace implements FSliderMarkStyleDelta {
  const _FSliderMarkStyleReplace(this._replacement);

  final FSliderMarkStyle _replacement;

  @override
  FSliderMarkStyle call(FSliderMarkStyle _) => _replacement;
}

class _FSliderMarkStyleMerge implements FSliderMarkStyleDelta {
  const _FSliderMarkStyleMerge({
    this.tickColor,
    this.tickSize,
    this.labelTextStyle,
    this.labelAnchor,
  ...
  FSliderMarkStyle call(FSliderMarkStyle original) => FSliderMarkStyle(
    tickColor: tickColor ?? original.tickColor,
