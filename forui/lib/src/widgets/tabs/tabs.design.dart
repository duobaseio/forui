...
sealed class FTabMotionDelta with Delta<FTabMotion> {
  /// Creates a complete replacement for a [FTabMotion].
  const factory FTabMotionDelta.replace(FTabMotion replacement) = _FTabMotionReplace;

  /// Creates a partial modification of a [FTabMotion].
  const factory FTabMotionDelta.merge({Duration? duration, Curve? curve}) = _FTabMotionMerge;
}

class _FTabMotionReplace implements FTabMotionDelta {
  const _FTabMotionReplace(this._replacement);

  final FTabMotion _replacement;

  @override
  FTabMotion call(FTabMotion _) => _replacement;
}

class _FTabMotionMerge implements FTabMotionDelta {
  const _FTabMotionMerge({this.duration, this.curve});
...
sealed class FTabsStyleDelta with Delta<FTabsStyle> {
  /// Creates a complete replacement for a [FTabsStyle].
  const factory FTabsStyleDelta.replace(FTabsStyle replacement) = _FTabsStyleReplace;

  /// Creates a partial modification of a [FTabsStyle].
  const factory FTabsStyleDelta.merge({
    BoxDecorationDelta? decoration,
    EdgeInsetsGeometry? padding,
    TextStyleDelta? selectedLabelTextStyle,
    TextStyleDelta? unselectedLabelTextStyle,
    BoxDecorationDelta? indicatorDecoration,
    FTabBarIndicatorSize Function()? indicatorSize,
    double? height,
    double? spacing,
...
class _FTabsStyleReplace implements FTabsStyleDelta {
  const _FTabsStyleReplace(this._replacement);

  final FTabsStyle _replacement;

  @override
  FTabsStyle call(FTabsStyle _) => _replacement;
}

class _FTabsStyleMerge implements FTabsStyleDelta {
  const _FTabsStyleMerge({
    this.decoration,
    this.padding,
  ...
  final TextStyleDelta? selectedLabelTextStyle;

  final TextStyleDelta? unselectedLabelTextStyle;

  final BoxDecorationDelta? indicatorDecoration;

  final FTabBarIndicatorSize Function()? indicatorSize;
  ...
  FTabsStyle call(FTabsStyle original) => FTabsStyle(
    ...
    selectedLabelTextStyle:
        selectedLabelTextStyle?.call(original.selectedLabelTextStyle) ?? original.selectedLabelTextStyle,
    unselectedLabelTextStyle:
        unselectedLabelTextStyle?.call(original.unselectedLabelTextStyle) ?? original.unselectedLabelTextStyle,
    indicatorDecoration: indicatorDecoration?.call(original.indicatorDecoration) ?? original.indicatorDecoration,
    indicatorSize: indicatorSize == null ? original.indicatorSize : indicatorSize!(),
