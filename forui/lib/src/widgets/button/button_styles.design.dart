...
/// A delta that applies modifications to a [FButtonStyles].
sealed class FButtonStylesDelta with Delta<FButtonStyles> {
  /// Creates a complete replacement for a [FButtonStyles].
  const factory FButtonStylesDelta.replace(FButtonStyles replacement) = _FButtonStylesReplace;

  /// Creates a partial modification of a [FButtonStyles].
  const factory FButtonStylesDelta.merge({
    FButtonStyleDelta? primary,
    FButtonStyleDelta? secondary,
    FButtonStyleDelta? destructive,
    FButtonStyleDelta? outline,
    FButtonStyleDelta? ghost,
  }) = _FButtonStylesMerge;
}

class _FButtonStylesReplace implements FButtonStylesDelta {
  const _FButtonStylesReplace(this._replacement);

  final FButtonStyles _replacement;

  @override
  FButtonStyles call(FButtonStyles _) => _replacement;
}

class _FButtonStylesMerge implements FButtonStylesDelta {
  const _FButtonStylesMerge({this.primary, this.secondary, this.destructive, this.outline, this.ghost});

  final FButtonStyleDelta? primary;

  final FButtonStyleDelta? secondary;

  final FButtonStyleDelta? destructive;

  final FButtonStyleDelta? outline;

  final FButtonStyleDelta? ghost;

  @override
  FButtonStyles call(FButtonStyles original) => FButtonStyles(
    primary: primary?.call(original.primary) ?? original.primary,
    secondary: secondary?.call(original.secondary) ?? original.secondary,
    destructive: destructive?.call(original.destructive) ?? original.destructive,
    outline: outline?.call(original.outline) ?? original.outline,
    ghost: ghost?.call(original.ghost) ?? original.ghost,
  );
}
