...
sealed class FButtonStyleDelta with Delta<FButtonStyle> {
  /// Creates a complete replacement for a [FButtonStyle].
  const factory FButtonStyleDelta.replace(FButtonStyle replacement) = _FButtonStyleReplace;

  /// Creates a partial modification of a [FButtonStyle].
  const factory FButtonStyleDelta.merge({
    FWidgetStateMap<BoxDecoration>? decoration,
...
class _FButtonStyleReplace implements FButtonStyleDelta {
  const _FButtonStyleReplace(this._replacement);

  final FButtonStyle _replacement;

  @override
  FButtonStyle call(FButtonStyle _) => _replacement;
}

class _FButtonStyleMerge implements FButtonStyleDelta {
  const _FButtonStyleMerge({
    this.decoration,
    this.contentStyle,
