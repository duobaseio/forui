...
sealed class FPaginationStyleDelta with Delta<FPaginationStyle> {
  /// Creates a complete replacement for a [FPaginationStyle].
  const factory FPaginationStyleDelta.replace(FPaginationStyle replacement) = _FPaginationStyleReplace;

  /// Creates a partial modification of a [FPaginationStyle].
  const factory FPaginationStyleDelta.merge({
    EdgeInsets? itemPadding,
    BoxConstraints? itemConstraints,
    FWidgetStateMap<IconThemeData>? itemIconStyle,
    FWidgetStateMap<BoxDecoration>? itemDecoration,
  ...
  }) = _FPaginationStyleMerge;
}

class _FPaginationStyleReplace implements FPaginationStyleDelta {
  const _FPaginationStyleReplace(this._replacement);

  final FPaginationStyle _replacement;

  @override
  FPaginationStyle call(FPaginationStyle _) => _replacement;
}

class _FPaginationStyleMerge implements FPaginationStyleDelta {
  const _FPaginationStyleMerge({
    this.itemPadding,
  ...
  FPaginationStyle call(FPaginationStyle original) => FPaginationStyle(
    itemPadding: itemPadding ?? original.itemPadding,
    itemConstraints: itemConstraints ?? original.itemConstraints,
    itemIconStyle: itemIconStyle ?? original.itemIconStyle,
    itemDecoration: itemDecoration ?? original.itemDecoration,
    itemTextStyle: itemTextStyle ?? original.itemTextStyle,
