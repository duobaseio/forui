...
sealed class FItemGroupStyleDelta with Delta<FItemGroupStyle> {
  /// Creates a complete replacement for a [FItemGroupStyle].
  const factory FItemGroupStyleDelta.replace(FItemGroupStyle replacement) = _FItemGroupStyleReplace;

  /// Creates a partial modification of a [FItemGroupStyle].
  const factory FItemGroupStyleDelta.merge({
    BoxDecorationDelta? decoration,
    double? spacing,
    FWidgetStateMap<Color>? dividerColor,
    double? dividerWidth,
    FItemStyleDelta? itemStyle,
  }) = _FItemGroupStyleMerge;
}

class _FItemGroupStyleReplace implements FItemGroupStyleDelta {
  const _FItemGroupStyleReplace(this._replacement);

  final FItemGroupStyle _replacement;

  @override
  FItemGroupStyle call(FItemGroupStyle _) => _replacement;
}

class _FItemGroupStyleMerge implements FItemGroupStyleDelta {
  const _FItemGroupStyleMerge({this.decoration, this.spacing, this.dividerColor, this.dividerWidth, this.itemStyle});
