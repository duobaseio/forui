...
sealed class FSelectScrollHandleStyleDelta with Delta<FSelectScrollHandleStyle> {
  /// Creates a complete replacement for a [FSelectScrollHandleStyle].
  const factory FSelectScrollHandleStyleDelta.replace(FSelectScrollHandleStyle replacement) =
      _FSelectScrollHandleStyleReplace;

  /// Creates a partial modification of a [FSelectScrollHandleStyle].
  const factory FSelectScrollHandleStyleDelta.merge({
    IconThemeDataDelta? iconStyle,
    Color? background,
    Duration? enterDuration,
    double? pixelsPerSecond,
  }) = _FSelectScrollHandleStyleMerge;
}

class _FSelectScrollHandleStyleReplace implements FSelectScrollHandleStyleDelta {
  const _FSelectScrollHandleStyleReplace(this._replacement);

  final FSelectScrollHandleStyle _replacement;

  @override
  FSelectScrollHandleStyle call(FSelectScrollHandleStyle _) => _replacement;
}

class _FSelectScrollHandleStyleMerge implements FSelectScrollHandleStyleDelta {
  const _FSelectScrollHandleStyleMerge({this.iconStyle, this.background, this.enterDuration, this.pixelsPerSecond});
