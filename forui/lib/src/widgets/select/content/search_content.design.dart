...
sealed class FSelectSearchStyleDelta with Delta<FSelectSearchStyle> {
  /// Creates a complete replacement for a [FSelectSearchStyle].
  const factory FSelectSearchStyleDelta.replace(FSelectSearchStyle replacement) = _FSelectSearchStyleReplace;

  /// Creates a partial modification of a [FSelectSearchStyle].
  const factory FSelectSearchStyleDelta.merge({
    FTextFieldStyleDelta? fieldStyle,
    FDividerStyleDelta? dividerStyle,
    FCircularProgressStyleDelta? progressStyle,
  }) = _FSelectSearchStyleMerge;
}

class _FSelectSearchStyleReplace implements FSelectSearchStyleDelta {
  const _FSelectSearchStyleReplace(this._replacement);

  final FSelectSearchStyle _replacement;

  @override
  FSelectSearchStyle call(FSelectSearchStyle _) => _replacement;
}

class _FSelectSearchStyleMerge implements FSelectSearchStyleDelta {
  const _FSelectSearchStyleMerge({this.fieldStyle, this.dividerStyle, this.progressStyle});
