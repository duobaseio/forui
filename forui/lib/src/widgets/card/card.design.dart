...
sealed class FCardStyleDelta with Delta<FCardStyle> {
  /// Creates a complete replacement for a [FCardStyle].
  const factory FCardStyleDelta.replace(FCardStyle replacement) = _FCardStyleReplace;

  /// Creates a partial modification of a [FCardStyle].
  const factory FCardStyleDelta.merge({BoxDecorationDelta? decoration, FCardContentStyleDelta? contentStyle}) =
      _FCardStyleMerge;
}

class _FCardStyleReplace implements FCardStyleDelta {
  const _FCardStyleReplace(this._replacement);

  final FCardStyle _replacement;

  @override
  FCardStyle call(FCardStyle _) => _replacement;
}

class _FCardStyleMerge implements FCardStyleDelta {
  const _FCardStyleMerge({this.decoration, this.contentStyle});
