...
sealed class FTimeFieldStyleDelta with Delta<FTimeFieldStyle> {
  /// Creates a complete replacement for a [FTimeFieldStyle].
  const factory FTimeFieldStyleDelta.replace(FTimeFieldStyle replacement) = _FTimeFieldStyleReplace;

  /// Creates a partial modification of a [FTimeFieldStyle].
  const factory FTimeFieldStyleDelta.merge({
    FTextFieldStyleDelta? fieldStyle,
    FPopoverStyleDelta? popoverStyle,
    FTimePickerStyleDelta? pickerStyle,
  }) = _FTimeFieldStyleMerge;
}

class _FTimeFieldStyleReplace implements FTimeFieldStyleDelta {
  const _FTimeFieldStyleReplace(this._replacement);

  final FTimeFieldStyle _replacement;

  @override
  FTimeFieldStyle call(FTimeFieldStyle _) => _replacement;
}

class _FTimeFieldStyleMerge implements FTimeFieldStyleDelta {
  const _FTimeFieldStyleMerge({this.fieldStyle, this.popoverStyle, this.pickerStyle});
