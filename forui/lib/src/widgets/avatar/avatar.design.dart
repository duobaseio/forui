...
sealed class FAvatarStyleDelta with Delta<FAvatarStyle> {
  /// Creates a complete replacement for a [FAvatarStyle].
  const factory FAvatarStyleDelta.replace(FAvatarStyle replacement) = _FAvatarStyleReplace;

  /// Creates a partial modification of a [FAvatarStyle].
  const factory FAvatarStyleDelta.merge({
    Color? backgroundColor,
    Color? foregroundColor,
...
class _FAvatarStyleReplace implements FAvatarStyleDelta {
  const _FAvatarStyleReplace(this._replacement);

  final FAvatarStyle _replacement;

  @override
  FAvatarStyle call(FAvatarStyle _) => _replacement;
}

class _FAvatarStyleMerge implements FAvatarStyleDelta {
  const _FAvatarStyleMerge({this.backgroundColor, this.foregroundColor, this.textStyle, this.fadeInDuration});
