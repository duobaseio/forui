...
sealed class FTappableStyleDelta with Delta<FTappableStyle> {
  /// Creates a complete replacement for a [FTappableStyle].
  const factory FTappableStyleDelta.replace(FTappableStyle replacement) = _FTappableStyleReplace;

  /// Creates a partial modification of a [FTappableStyle].
  const factory FTappableStyleDelta.merge({
    FWidgetStateMap<MouseCursor>? cursor,
    Duration? pressedEnterDuration,
    Duration? pressedExitDuration,
...
class _FTappableStyleReplace implements FTappableStyleDelta {
  const _FTappableStyleReplace(this._replacement);

  final FTappableStyle _replacement;

  @override
  FTappableStyle call(FTappableStyle _) => _replacement;
}

class _FTappableStyleMerge implements FTappableStyleDelta {
  const _FTappableStyleMerge({this.cursor, this.pressedEnterDuration, this.pressedExitDuration, this.motion});
...
sealed class FTappableMotionDelta with Delta<FTappableMotion> {
  /// Creates a complete replacement for a [FTappableMotion].
  const factory FTappableMotionDelta.replace(FTappableMotion replacement) = _FTappableMotionReplace;

  /// Creates a partial modification of a [FTappableMotion].
  const factory FTappableMotionDelta.merge({
    Duration? bounceDownDuration,
    Duration? bounceUpDuration,
    Curve? bounceDownCurve,
    Curve? bounceUpCurve,
    Animatable<double>? bounceTween,
  }) = _FTappableMotionMerge;
}

class _FTappableMotionReplace implements FTappableMotionDelta {
  const _FTappableMotionReplace(this._replacement);

  final FTappableMotion _replacement;

  @override
  FTappableMotion call(FTappableMotion _) => _replacement;
}

class _FTappableMotionMerge implements FTappableMotionDelta {
  const _FTappableMotionMerge({
