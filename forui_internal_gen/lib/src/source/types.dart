...
Future<(String type, String assignment, String? sentinel)> deltaField(
  ...
  if (field.type case InterfaceType(:final element) when element.name == 'FVariants') {
    final type = await declaredType(step, field);
...
///
/// `FVariants<K, E, V, D>` has a circular dependency where K (the constraint type) and E (the variant type) are
...
Future<String> fieldType(BuildStep step, FieldElement field) async {
  if (field.type case InterfaceType(:final element) when element.name == 'FVariants') {
    if (await declaredType(step, field) case final type?) {
...
/// Returns the type annotation of [field]'s declaration, or `null` if it is unavailable.
///
/// The type is read from the AST rather than the element model because generated types may not be resolved yet.
///
/// A field declared by a primary constructor is induced by one of its formal parameters rather than by a field
/// declaration, so it has no [VariableDeclarationList] to read the type from.
Future<TypeAnnotation?> declaredType(BuildStep step, FieldElement field) async {
  if (await step.resolver.astNodeFor(field.firstFragment) case final node?) {
    if (node.parent case VariableDeclarationList(:final type?)) {
      return type;
    }
  }

  if (field.declaringFormalParameter?.firstFragment case final fragment?) {
    if (await step.resolver.astNodeFor(fragment) case RegularFormalParameter(:final type?)) {
      return type;
    }
  }
