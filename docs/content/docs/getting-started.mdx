...
This means that that following entry in your `pubspec.yaml` file will **not** automatically upgrade to `0.25.0`:

```yaml filename="pubspec.yaml"
dependencies:
  forui: ^0.24.0 // ❌ will not upgrade to 0.25.0
```
...
<Callout type="info">
    Forui Lucide is bundled with the forui package. You don't need to install it separately if you install `forui`.
</Callout>
...
To generate a basic Forui app structure in your project, run the command below.
...
variant. To generate a custom theme, run [`dart run forui theme create`](/docs/reference/cli#create-2) or design one
visually in [Forui Create](https://create.forui.dev).
