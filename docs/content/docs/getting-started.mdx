...
This means that that following entry in your `pubspec.yaml` file will **not** automatically upgrade to `0.24.0`:

```yaml filename="pubspec.yaml"
dependencies:
  forui: ^0.23.0 // ❌ will not upgrade to 0.24.0
```
...
To generate a basic Forui app structure in your project, run the command below. Alternatively, pass a `--preset` from
[create.forui.dev](https://create.forui.dev) to customize the generated theme.
...
Forui ships with a built-in `FTheme.neutral` theme that you can use out of the box. The example above uses its dark
variant. To generate a custom theme with your own colors, fonts, icons, and border radius, use
[create.forui.dev](https://create.forui.dev) or `dart run forui theme create`.
